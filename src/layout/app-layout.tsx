...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['guest-user'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<TeamInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/teams` : null),
    () => getTeams({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Teams',
      path: isTenantUser ? `/teams/view/${data?.[0]?.id}` : '/teams',
      entity: 'team',
    ...
    { name: 'Error Logs', path: '/error-logs', entity: 'error_logs', service: AccessServiceEnum.PROJECT, icon: FiBox },
    {
      name: 'Instructions',
      path: '/instructions',
      entity: 'instruction',
    ...
    {
      name: 'Love Calculations',
      path: '/love-calculations',
      entity: 'love_calculations',
      service: AccessServiceEnum.PROJECT,
      icon: FiBox,
    },
    { name: 'Resets', path: '/resets', entity: 'reset', service: AccessServiceEnum.PROJECT, icon: FiBox },
    { name: 'Shares', path: '/shares', entity: 'share', service: AccessServiceEnum.PROJECT, icon: FiBox },
    {
      name: 'Shared Results',
      path: '/shared-results',
