...
import { getResetById } from 'apiSdk/resets';
...
function ResetViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ResetInterface>(
    () => (id ? `/resets/${id}` : null),
    () =>
      getResetById(id, {
        relations: ['user'],
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('reset', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/resets/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem
                  label="Date Reset"
                  text={data?.date_reset ? format(parseISO(data?.date_reset as unknown as string), 'dd-MM-yyyy') : ''}
                />
