...
import { getUserPreferences } from 'apiSdk/user-preferences';
import { UserPreferencesInterface } from 'interfaces/user-preferences';
import { getShares } from 'apiSdk/shares';
import { ShareInterface } from 'interfaces/share';

import { DeviceInfoListPage as ShowCaseList } from 'pages/device-infos';

function DashboardPage() {
  const { session } = useSession();

  const compatibilityStat = useSWR<PaginatedInterface<CompatibilityInterface>>(
    () => `/compatibilities`,
    () => getCompatibilities({ limit: 1, offset: 0 }),
  );

  const userPreferencesStat = useSWR<PaginatedInterface<UserPreferencesInterface>>(
    () => `/user-preferences`,
    () => getUserPreferences({ limit: 1, offset: 0 }),
  );

  const shareStat = useSWR<PaginatedInterface<ShareInterface>>(
    () => `/shares`,
    () => getShares({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Compatibilities',
      result: compatibilityStat?.data?.totalCount,
      isLoading: compatibilityStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'User Preferences',
      result: userPreferencesStat?.data?.totalCount,
      isLoading: userPreferencesStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Shares',
      result: shareStat?.data?.totalCount,
      isLoading: shareStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`device-infos`} passHref legacyBehavior>
                  <Button
